...
def get_pub_date_tuple(soup, pub_type = "ppub"):
	"""
	Find the publishing date for populating, returning as a tuple (D, M, Y)
	...
	try:
		...
		if(len(pub_date_section) == 0):
			if(pub_type == "ppub"):
				pub_type = "epub"
			pub_date_section = extract_nodes(soup, "pub-date", attr = "pub-type", value = pub_type)
		tuple = get_ymd(pub_date_section[0])
...
def get_pub_date(soup, pub_type = "ppub"):
	...
	(day, month, year) = get_pub_date_tuple(soup, pub_type)
